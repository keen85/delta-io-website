...
description: This post explains how to create and change partitioned Delta tables making use of *generated* partition columns (especially for `DateType` and `TimestampType` partition columns)
thumbnail: ./xxx.png
...
- faster reads: when querying a partitioned Delta table using a filter predicated on one of the partition columns, Delta will tell the execution engine to skip parquet files of partitions that don't match the predicate entirely. This is called **partition pruning**.
  - https://delta.io/blog/pros-cons-hive-style-partionining/#:~:text=Hive%2Dstyle%20partitioning%20for%20data%20lakes
- isolation for concurrent transactions: when working on a partitioned table, **concurrent `INSERT`, `DELETE`, `UPDATE` and `MERGE` operations will succeed** if they use predicates on partition columns that do not overlap.
  - https://delta.io/blog/pros-cons-hive-style-partionining/#:~:text=Hive%2Dstyle%20partitioning%20for%20concurrency
  - https://docs.delta.io/latest/concurrency-control.html#-avoid-conflicts-using-partitioning-and-disjoint-command-conditions

However, there is also a danger of over-partitioning a table, meaning ending up with many partitions containing only small files, which will slow down read operations.
- https://delta.io/blog/pros-cons-hive-style-partionining/#:~:text=Hive%2Dstyle%20partitioning%20can%20exacerbate%20the%20small%20file%20problem
...
# ✅ partition by *binned* `TimestampType` column (using generation expression)
A better approach is, to ["bin"](https://en.wikipedia.org/wiki/Data_binning) the TimestampType values to a *coarser* granularity (e.g. hourly, daily, yearly).
This can easily be done by a **generated column** making use of the [`DATE_TRUNC`](https://spark.apache.org/docs/latest/api/sql/#date_trunc) function.
Advantage of a generated column here is, that when appending data to the table after its creation, it will be calculated from the reference column automatically during insert (and the column does not need to be contained in the append DataFreame). Moreover, since Delta 2.4 when querying the table with a predicate on a column **referenced by the partition column's generation expression** will also support partition pruning (e.g. `SELECT * FROM events WHERE event_timestamp = '1990-06-15 09:01:01'` will tell the engine only to read partition `event_timestamp_bin=1990-01-01` initially and then filter for `event_timestamp = '1990-06-15 09:01:01'` in the next example)
- https://delta.io/blog/2023-04-12-delta-lake-generated-columns/
- https://docs.delta.io/latest/delta-batch.html#-deltausegeneratedcolumns
...
In this example, the read the Delta table previously binned by year, transform the DataFrame and and simply overwrite the Delta table in-place. Due to Delta's versioning / time travel functionality, this will create a new version of the table with the changed generation expression. So there is no need to create a temporary table and swap it afterwards. 
```python
spark.table("events")
...
# Parting thoughts
TODO: Liquid Clustering to the rescue 
